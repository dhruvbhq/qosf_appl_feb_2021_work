...
def launch_tests():
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 30 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 31 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 32 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 33 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 34 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 35 failed."
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 36 failed."
    q1.measure_ckt(num_shots=2000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 0,
    ...
    q1 = siqcandar.siqc_ckt(name="q1", num_qubits=2)
    q1.init_state(init_type="GROUND")
    psi1 = np.array([0.+0.j, 0.+0.j, 0.+1.j, 0.+0.j])
    program = [{"gate_type" : "Y", "target" : 1}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 37 failed."
    q1.measure_ckt(num_shots=2000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 0,
    ...
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    psi1 = (1/np.sqrt(2))*(ket00 - ket11)
    program = [{"gate_type" : "Z", "target" : 1}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 38 failed."
    q1.measure_ckt(num_shots=2000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 1000,
    ...
    psi_init = (1/np.sqrt(2))*(ket00 + ket11)
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    psi1 = (1/np.sqrt(2))*(ket00 + (ket11*1j))
    program = [{"gate_type" : "S", "target" : 1}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 39 failed."
    q1.measure_ckt(num_shots=2000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 1000,
    ...
    psi_init = (1/np.sqrt(2))*(ket00 + ket11)
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    psi1 = np.array([0.70710678+0.j , 0.+0.j , 0.+0.j , 0.5-0.5j])
    program = [{"gate_type" : "T", "target" : 1}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-8)), "Test 40 failed."
    q1.measure_ckt(num_shots=2000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 1000,
    ...
    psi1 = np.array([0.05416408-0.61863348j, 0.77779859+0.09689285j])
    program = [{"gate_type" : "H", "target" : 0}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-6)), "Test 41 failed."
    ...
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    #This is the expected output
    psi1 = np.array([ 0.13843442+0.14799115j, -0.33023773-0.68795715j,
        0.03492841+0.02857399j,  0.54990028+0.26863685j])
    program = [{"gate_type" : "H", "target" : 0}]
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-6)), "Test 42 failed."
    ...
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    #This is the expected output
    psi1 = np.array([-0.09790776+0.36246208j, -0.33935261-0.06519143j,
    ...
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-6)), "Test 43 failed."
    q1.measure_ckt(num_shots=100000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"0000" : 14096, 
    ...
    psi1 = np.array([0.70710678+0.j, 0.35355339+0.61237244j])
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-6)), "Test 44 failed."
    q1.measure_ckt(num_shots=1000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"0" : 500,
    ...
    q1.init_state(init_type="CUSTOM", st_vec=psi_init)
    #This is the expected output state
    psi1 = np.array([0.3080127 -0.21650635j, -0.09150635+0.59150635j,
                     0.3080127 -0.21650635j, -0.09150635+0.59150635j])
    q1.execute_ckt(program)
    for i in range(len(psi1)):
        assert(np.linalg.norm(psi1[i] - q1.st_vec[i]) < 10**(-6)), "Test 45 failed."
    q1.measure_ckt(num_shots=10000)
    #If measured frequency are off by more than 10% compared to expected 
    #frequency, issue an error
    exp_prob = {"00" : 1420,
